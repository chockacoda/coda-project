...
CREATE DEFINER=`root`@`localhost` TRIGGER `rating` AFTER INSERT ON `movie` FOR EACH ROW BEGIN
    IF NEW.movie_id  IS NOT NULL THEN
        INSERT INTO rating(movie_id, user_id,star)
        VALUES(new.movie_id,2,null);
    END IF;
END
...
-- Dump completed on 2020-12-15 17:00:06
